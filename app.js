...
app.controller("WordsController", ["$scope", "$routeParams", "LSTMService", function($scope, $routeParams, LSTMService) {
  console.log($routeParams.n);
  var n = parseInt($routeParams.n === undefined ? 0 : $routeParams.n);
  
  var colorizer = d3.interpolateRdBu;    

  d3.json("imdb_reviews.json", function(data) {        
    for (var i = 0; i < 50; i++) {
      d = _.filter(data, function(x) { return x["neuron"] == n; });
      console.log(d[i]);
      LSTMService.buildWordSequence(d[i]["review"], d[i]["weights"], colorizer);
    }
  });
}]);


app.controller("Words2Controller", ["$scope", "$routeParams", "LSTMService", function($scope, $routeParams, LSTMService) {
  console.log($routeParams.n);
  var n = parseInt($routeParams.n === undefined ? 0 : $routeParams.n);
  ...
  d3.json("imdb_reviews.json", function(data) {        
    console.log("HMM");
    d = _.filter(data, function(x) { return x["review_index"] == n; });
    for (var i = 0; i < 50; i++) {
      LSTMService.buildWordSequence(d["review"], d["weights"], colorizer);
    }
  });
}]);
